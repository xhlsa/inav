 ...
 *
 * INAV port notes:
 *  - Receiver pins come from target defines; settings live in rxExpressLrsSpiConfig.
 *  - The UID is derived from expresslrs_bind_phrase when set.
 *  - Only accepted RC_DATA packets are reported to INAV as new RC frames. Sync
 *    and MSP packets keep the link statistics alive but never refresh the RC
 *    signal timeout, so stale sticks cannot outlive a real RC link.
 *  - Configuration is never saved while armed.
 *  - MSP over the ELRS link is not ported.
 */
...
// MSP over the ELRS link is not ported (INAV's CRSF MSP helpers differ)
#undef USE_ELRS_MSP_OVER_TELEMETRY
...
static rxRuntimeConfig_t *elrsRxRuntimeConfig;
static uint8_t bindPhraseUID[6];
...
static volatile uint8_t dmaBuffer[ELRS_RX_TX_BUFF_SIZE];
static volatile uint8_t telemetryPacket[ELRS_RX_TX_BUFF_SIZE];
...
#ifdef USE_ELRS_MSP_OVER_TELEMETRY
...
static void unpackAnalogChannelData(uint16_t *rcData, volatile elrsOtaPacket_t const * const otaPktPtr)
{
    ...
    rcData[ELRS_ARM_V3_CHANNEL] = convertSwitch1b(otaPktPtr->rc.ch4);
}
...
static void setLinkStatsDisconnected(void)
{
    rxLinkStatistics.uplinkRSSI = -130;
    rxLinkStatistics.uplinkLQ = 0;
    rxLinkStatistics.uplinkSNR = -30;
    if (elrsRxRuntimeConfig) {
        lqTrackerSet(elrsRxRuntimeConfig->lqTracker, 0);
    }
}
...
static void setRfLinkRate(const uint8_t index)
{
    ...
    receiver.config(
        ...
        receiver.inBindingMode || (receiver.UID[5] & 0x01),
...
#ifdef USE_ELRSV3
    rxLinkStatistics.rfMode = (uint8_t)receiver.modParams->enumRate;
#else
    rxLinkStatistics.rfMode = (uint8_t)receiver.modParams->v4Rate;
#endif
}
...
static void expressLrsSendTelemResp(void)
{
    ...
    if (nextTelemetryType == ELRS_TELEMETRY_TYPE_LINK || !isTelemetrySenderActive()) {
        ...
        otaPkt.tlm_dl.ul_link_stats.stats.antenna = 0;
...
static void OtaUpdateCrcInitFromUid(void)
{
    if (receiver.inBindingMode) {
        ...
        crcInitializer ^= (uint16_t)ELRS_OTA_VERSION_ID << 8;
#endif
    }
}
...
{
    // A UID received over the air replaces any configured binding phrase
    rxExpressLrsSpiConfigMutable()->bindPhrase[0] = '\0';
...
rx_spi_received_e processRFPacket(volatile uint8_t *payload, uint32_t timeStampUs)
{
    ...
    rx_spi_received_e receivedStatus = RX_SPI_RECEIVED_LINK;
    uint32_t timeStampMs = millis();
    ...
    case ELRS_RC_DATA_PACKET:
        ...
        if (receiver.connectionState == ELRS_CONNECTED && connectionHasModelMatch) {
...
#else
                bool telemInEveryPacket = true;
#endif
                if (telemInEveryPacket || wideSwitchIndex == 7) {
            ...
            receivedStatus = RX_SPI_RECEIVED_DATA;
        }
        break;
...
//setup
static void expressLrsExtiHandler(void)
{
    expressLrsISR(true);
}
...
bool expressLrsSpiInit(const rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig)
{
    ...
    if (!rxSpiDeviceInit() || !rxSpiExtiConfigured()) {
        return false;
    }

    elrsRxRuntimeConfig = rxRuntimeConfig;
    rxRuntimeConfig->channelCount = ELRS_MAX_CHANNELS;

#ifdef RX_EXPRESSLRS_SPI_RESET_PIN
    receiver.resetPin = IOGetByTag(IO_TAG(RX_EXPRESSLRS_SPI_RESET_PIN));
#else
    receiver.resetPin = IO_NONE;
#endif

#ifdef RX_EXPRESSLRS_SPI_BUSY_PIN
    receiver.busyPin = IOGetByTag(IO_TAG(RX_EXPRESSLRS_SPI_BUSY_PIN));
#else
    ...
    if (rxExpressLrsSpiConfig()->bindPhrase[0]) {
        elrsUidFromBindPhrase(rxExpressLrsSpiConfig()->bindPhrase, bindPhraseUID);
        receiver.inBindingMode = false;
        receiver.UID = bindPhraseUID;
    } else if (rxExpressLrsSpiConfig()->UID[0] || rxExpressLrsSpiConfig()->UID[1]
    ...
    if (!expressLrsInitialiseTimer(RX_EXPRESSLRS_TIMER_INSTANCE)) {
        return false;
    }
    ...
    rxSpiExtiInit(expressLrsExtiHandler);
    setLinkStatsDisconnected();
...
static void handleConfigUpdate(const uint32_t timeStampMs)
{
    if ((timeStampMs - receiver.configCheckedAtMs) > ELRS_CONFIG_CHECK_MS) {
        receiver.configCheckedAtMs = timeStampMs;
        // Writing flash stalls the CPU; never do it in flight. Retried once disarmed.
        if (receiver.configChanged && !ARMING_FLAG(ARMED)) {
            saveConfigAndNotify();
...
static void handleLinkStatsUpdate(const uint32_t timeStampMs)
{
    if ((timeStampMs - receiver.statsUpdatedAtMs) > ELRS_LINK_STATS_CHECK_MS) {
        ...
        if (receiver.connectionState == ELRS_CONNECTED) {
            ...
            uint16_t rssiScaled = scaleRange(constrain(receiver.rssiFiltered, receiver.rfPerfParams->sensitivity, -50), receiver.rfPerfParams->sensitivity, -50, 0, RSSI_MAX_VALUE);
            lqTrackerSet(elrsRxRuntimeConfig->lqTracker, rssiScaled);

            rxLinkStatistics.uplinkRSSI = receiver.rssiFiltered;
            rxLinkStatistics.uplinkLQ = receiver.uplinkLQ;
            rxLinkStatistics.uplinkSNR = receiver.snr / 4;
            rxLinkStatistics.uplinkTXPower = txPowerIndexToValue(txPower);
            rxLinkStatistics.activeAntenna = 0;
        } else {
            setLinkStatsDisconnected();
        }
    }
...
rx_spi_received_e expressLrsDataReceived(uint8_t *payloadBuffer)
{
    ...
    receiver.inBindingMode ? rxSpiLedBlinkBind() : rxSpiLedBlinkRxLoss(rfPacketStatus != RX_SPI_RECEIVED_NONE);
